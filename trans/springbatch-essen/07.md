...
在前一章中，我们学习了企业集成、各种企业应用程序集成，以及用消息驱动方法集成 Spring 批处理应用程序的 Spring Integration 项目。我们还学习了使用 Spring 集成和 RESTful 作业处理技术启动批处理作业。Spring 批处理作业执行处理随时间变化的大量数据。这种不断变化的数据有时可能会损坏，并导致作业执行失败。密切关注此类故障非常重要，并且应该以建设性的方式保存故障原因，以便将来跟踪和修复。
...
到目前为止，我们已经看到了各种批处理作业配置和执行处理来自不同来源的数据，对其进行处理，并将结果推送到另一个数据存储中。只要作业继续按照我们进行配置的方式执行，一切看起来都很好。应用程序的稳定性可以通过应用程序对其周围环境(即应用程序运行的环境、外部系统的可用性和可访问性以及提供给应用程序的数据的正确性)的任何问题的响应的强度和详细程度来衡量。

应用程序应该能够生成清晰的跟踪信息，包括应用程序内部和外部发生的功能、谁在使用它、性能如何以及应用程序面临的一系列问题/错误。Spring Batch 处理这些参数，并生成一个更好的基础设施来监控批处理作业处理并存储这些监控信息。
...
Spring 批处理作业的执行可以通过 Spring 的开源项目提供的 web 接口来监控和检查，即 **Spring Batch Admin** 。这是一个用 Spring MVC 用户界面构建的简单 web 应用程序，作为 Spring 批处理应用程序和系统的管理控制台。这个项目中开发的主要用例是检查作业、启动作业、检查执行和停止执行。
