...
我们从前面的章节中了解到，SpringBatch作业在一个名为批处理作业元数据的上下文中维护关于作业执行的信息。我们可以使用这个上下文在步骤之间共享数据。基于键值的数据在使用时由`org.springframework.batch.item.ExecutionContext`维护。下面是放入/从中获取数据的方法:
