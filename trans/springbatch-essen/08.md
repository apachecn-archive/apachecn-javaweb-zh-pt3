...
Spring Batch 提供了以下方式来扩展批处理应用程序:
...
通过本章，我们了解了性能和批量缩放的重要性。我们还学习了 Spring 批处理产品来扩展批处理应用程序。此外，我们还了解了线程模型、并行处理、远程分块和分区技术的细节和配置。我们已经了解了如何选择正确的策略来利用可用的基础设施扩展批处理应用程序，从而完成了本章的学习。

在下一章，我们将详细了解在 Spring 批处理应用程序上执行不同类型的测试。