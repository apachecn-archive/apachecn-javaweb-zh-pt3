...
下面是启用了 SpringBatch集成的 Spring XML 应用上下文文件:
...
通过本章，我们了解了企业集成和企业应用集成的不同方法。我们还了解了 Spring Integration 项目如何将 SpringBatch应用与它的消息驱动方法集成在一起。我们还学习了通过从应用上下文访问 Spring Integration 组件来启动批处理作业。我们在理解 RESTful 作业处理技术的基础上完成了这一章。

在下一章，我们将学习如何检查 SpringBatch作业，包括访问执行数据、监听器和 web 监控。