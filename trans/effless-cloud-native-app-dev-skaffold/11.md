...
在本节中，我们将创建一个 Spring Boot 应用，显示您当前位置的实时空气质量数据。我们会用open AQ(https://open AQ . org/)提供的 API，这是一个叫做空气质量数据维基百科的非营利组织。它公开了许多实时空气质量数据的端点，但是我们将为我们的 Spring Boot 应用使用 https://api.openaq.org/v1/latest?country=IN[的 URL。我们开始吧。](https://api.openaq.org/v1/latest?country=IN)
