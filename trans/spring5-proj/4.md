...
Spring Security 是一个高度全面和可扩展的框架；它为基于 J2EE 的 web 和 REST 应用程序提供身份验证和授权支持。我们将看看如何将 Spring Security 与 LDAP 集成在一起以执行身份验证。根据定义，**认证**是一种验证或决定个人或实体是否如其所声称的那样的机制。
...
成功注册该应用程序后，Google 将以客户端 ID 和客户端密码的形式创建客户端凭据，如下图所示:
...
在上一节中，Google 使用了授权码授权类型。我们将在本节中实现一个隐式授权类型。让我们回忆一下，隐式授权类型是专门为 JavaScript 应用程序设计的。由于它在浏览器中运行，授权服务器直接发送访问令牌。出于安全目的，不支持刷新令牌。
...
REST 控制器配置了 path `/api`，这意味着它的所有方法都可以通过有效的授权令牌进行访问。现在，是运行应用程序和测试流程的时候了。我们将首先运行资源服务器应用程序( [http://localhost:8082](http://localhost:8082) ，假设它正在端口`8082`上运行)。它将显示一个链接，该链接会将流重定向到授权服务器。
...
这就是使用定制授权服务器实现隐式授权类型的方式。使用隐式授权类型时，需要注意一些事情。因为它是为 JavaScript 应用程序设计的，所以授权和资源服务器都应该可以安全地访问(通过 HTTPS)。第二件事是，在隐式授权类型中，访问令牌由授权服务器直接返回给浏览器，而不是在可信的后端；强烈建议配置短期访问令牌，以降低访问令牌泄露的风险。
...
安全性是任何系统的重要组成部分。它的有效性取决于各个方面，比如简单性、特性丰富性、与其他系统集成的容易程度、灵活性、健壮性等等。这一整章都是基于 Spring Security 的。这是一个成熟的框架，用于保护基于 J2EE 的应用程序。
...
除了使用 LDAP 进行身份验证，我们还探索了如何从 Spring 应用程序管理 LDAP 中的用户。我们使用 Spring 数据框架来实现这一点。接下来，我们为 LDAP 中的角色(权限)创建了一个结构。在相同的序列中，我们获取角色细节，并使用 LDAP 在 Spring Security 中实现授权。
...
在下一章，我们将探索另一个工具，叫做 JHipster。它是一个开源的应用程序生成器框架，主要用于开发 web 应用程序和微服务，以响应式 web 前端(Angular 或 React)和 Spring 框架作为后端。