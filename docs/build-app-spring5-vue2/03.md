...
Spring Boot 对构建生产就绪型应用有自己的看法。它的观点主要通过它的启动器和它的自动配置机制来表达。正因为如此，大多数 Spring Boot 应用比传统的Spring应用需要更少的Spring配置。
