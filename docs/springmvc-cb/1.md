...
有许多不同的 JVM 实现，包括一些开源解决方案，如 OpenJDK 和 IcedTea (RedHat)。JVM 的选择实际上取决于应用的需求。根据经验和在生产中部署的参考实施，我们选择了 Oracle Hotspot;这个 JVM 实现可以被信任用于广泛的通用目的。如果你有运行 Java UI 应用的，热点也表现得很好。月食就是其中之一。
