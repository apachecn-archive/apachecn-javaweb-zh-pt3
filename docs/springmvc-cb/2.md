...
5.  Select both your parent projects and right-click on one of them. Go to **Team** | **Add to index**:
    ...
    ![Configuring GIT in Eclipse](graphics/B04049_02_01.jpg)

6.  From the top-right panel, click on the **Git** **perspective**:

    ![Configuring GIT in Eclipse](graphics/B04049_02_02.jpg)

    如果您还没有这个透视图，请使用![Configuring GIT in Eclipse](graphics/B4049_02_26.jpg)按钮添加它。
...
10.  As shown in the following screenshot, right-click and select **Checkout** to see the latest version of the branch **origin/v1.x.x**.
    ...
    ![Configuring GIT in Eclipse](graphics/B04049_02_03.jpg)

11.  When prompted, Checkout as **New Local Branch**:

    ![Configuring GIT in Eclipse](graphics/B04049_02_04.jpg)
...
12.  The actual workspace should now be synchronized with the branch v1.x.x. This branch reflects the state of the environment at the end of [Chapter 1](1.html "Chapter 1\. Setup Routine for an Enterprise Spring Application"), Setup *Routine for an Enterprise Spring Application.*
    ...
    ![Configuring GIT in Eclipse](graphics/B04049_02_05.jpg)
 ...
 ******![MVC design pattern](graphics/B04049_02_06.jpg)
...
![Spring MVC flow](graphics/B04049_02_07.jpg)
...
在我们的应用程序中，在 **cloudstreetmarket-webapp** 中，`DispatcherServlet`被命名为 spring，并且覆盖了应用程序的完整上下文路径:`/*`。
...
2.  Access the `http://localhost:8080/portal/whatever` or `http://localhost:8080/portal/index` URL with your browser.
    ...
    ![How to do it...](graphics/B04049_02_08.jpg)
...
这个带有`*`通配符的特殊案例允许 URI 在应用程序显示名称后以`/`开头的任何请求都有资格被这个方法处理。
...
![Getting ready](graphics/B04049_02_09.jpg)
...
![How to do it...](graphics/B04049_02_10.jpg)
...
2.  Inside this archive, you'll see a `css` directory, a `js` directory, an `img` directory, and finally an `index.html` file. Opening the `index.html` file with a web browser should render the following home page:
    ...
    ![Installing a Bootstrap theme](graphics/B04049_02_11.jpg)
...
4.  At this point, after all these changes, restarting Tomcat and calling the same URL `http://localhost:8080/portal/index` resulted in the following state:
    ...
    ![Customising a Bootstrap theme](graphics/B04049_02_12.jpg)
...
9.  Try to dynamically resize a browser window that renders: `http://localhost:8080/portal/index`. You should observe a responsive and adaptive style as in the following picture:
    ...
    ![Creating responsive content](graphics/B04049_02_13.jpg)
...
![Defining columns](graphics/B04049_02_14.jpg)
...
![Defining columns](graphics/B04049_02_15.jpg)
...
![Offsetting and nesting](graphics/B04049_02_16.jpg)
...
![Uniform Buttons](graphics/B04049_02_17.jpg)
...
![Icons](graphics/B04049_02_18.jpg)
...
![Icons](graphics/B04049_02_19.jpg)
...
![Navbars](graphics/B04049_02_20.jpg)
...
![Hero units](graphics/B04049_02_21.jpg)
...
![Alerts](graphics/B04049_02_22.jpg)
...
![Badges and labels](graphics/B04049_02_23.jpg)
...
![Request lifecycle](graphics/B04049_02_24.jpg)
...
在这份食谱中，我们解释了我们是如何摆脱先前在 JSP 中引入的渲染逻辑来构建 DOM 的。我们现在将依靠 AngularJS 来完成这项工作。
...
AngularJS 在 DOM 中查找一个`ng-app`声明，以便根据 DOM 元素绑定一个模块，并启动(自动引导)这个模块。每个 HTML 文档只能自动引导一个应用程序(或模块)。

如果需要，我们仍然可以为每个文档定义多个应用程序，并手动引导它们。但是 AngularJS 社区促使我们将应用程序绑定到 HTML 或 BODY 标签上。

#### 模块自动引导

我们的应用程序是自动引导的，因为它在 HTML 标签中被引用:
...
![AngularJS Controllers](graphics/B04049_02_25.jpg)
...
正如你所想象的，这是 AngularJS 的一个快速介绍。当我们的应用程序中有了 REST API 和更多的特性时，我们会在原位发现更多的东西。

AngularJS 变得非常受欢迎，一个活跃的社区正在支持它。它的核心思想和实现基于一个显式的 DOM，提供了一种与应用程序联系的彻底而简单的方法。
