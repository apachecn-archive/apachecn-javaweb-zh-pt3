...
14.  回到服务器端，我们还为金融交易实体创建了一个定制的验证器。这个验证器使用了Spring`ValidationUtils` :
...
首先，a `MessageSource`是Spring接口(`org.sfw.context.MessageSource`)。`MessageSource`对象负责解析来自不同参数的消息。
...
`LocaleResolver`是Spring接口(`org.springframework.web.servlet. LocaleResolver`):
