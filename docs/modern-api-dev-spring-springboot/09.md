...
在这一章中，你将学习集装箱化、Docker和 Kubernetes 的基础知识。然后，您将使用这些概念，通过 Docker 封装一个示例电子商务应用。然后，这个容器将被部署为 Kubernetes 集群。您将为 Kubernetes 使用 minikube，这使得学习和基于 Kubernetes 的开发更加容易。
