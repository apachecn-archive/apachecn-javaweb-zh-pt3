...
在本章中，我们将学习如何使用 Spring 数据对我们的数据库执行反应式操作。但是首先，我们将学习如何在我们的应用程序中使用和配置 MongoDB，这是本章选择的 NoSQL 数据库。然后，我们将看到如何使用之前创建的示例，轻松地将之前创建的 RESTful APIs 与 CRUD 存储库连接起来。通过这些最后的步骤，我们将准备好交付与 NoSQL 数据库完全集成的快速、无阻塞的反应式微服务。
...
您可能熟悉 SQL 数据库，因为它们可能是数百万个应用程序中最常用的数据库模型。但是 NoSQL 数据库由于其强大的功能而在行业中得到广泛应用，并且可以轻松集成到我们的微服务中。在本节中，我们将了解它们，以及如何使用 MongoDB 作为微服务所需数据的存储库。
