...
这告诉我们，通过将`spring-boot-starter-test`作为依赖项包含在我们的构建中，我们将自动获得`spring-test`、`spring-boot`、`junit`、`mockito`、`hamcrest-library`、`jsonassert`和`json-path`。这些库将为我们提供所有必要的东西，以便开始为我们将要开发的软件编写应用程序测试，而不需要手动地将这些依赖项单独添加到构建文件中。
